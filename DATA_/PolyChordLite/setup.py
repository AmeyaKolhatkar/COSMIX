...
if sys.platform != "darwin" and sys.platform != "win32":
    # Set RPATH on Linux machines only
    RPATH_FLAG += ["-Wl,-rpath,$ORIGIN/pypolychord/lib"]

FORTRAN_RUNTIME_FLAG = []
if sys.platform == "win32":
    FORTRAN_RUNTIME_FLAG += ["-lgfortran"]
...
class DistributionWithOption(Distribution, object):
    def __init__(self, *args, **kwargs):
        ...
        # Default to no-MPI on Windows since mpifort is not available
        self.no_mpi = True if sys.platform == "win32" else None
...
class CustomBuildPy(_build_py, object):
    def run(self):
        ...
        env.update({k : os.environ[k] for k in ["CC", "CXX", "FC", "TEMP", "TMP", "TMPDIR", "USERPROFILE", "APPDATA", "LOCALAPPDATA", "SYSTEMROOT"] if k in os.environ})
        subprocess.check_call(["make", "-e", "libchord.so"], env=env, cwd=BASE_PATH)
        # On Windows also build the static library for linking
        if sys.platform == "win32":
            subprocess.check_call(["make", "-e", "libchord.a"], env=env, cwd=BASE_PATH)
...
pypolychord_module = Extension(
        ...
        libraries=['chord',],
        extra_link_args=RPATH_FLAG + CPPRUNTIMELIB_FLAG + FORTRAN_RUNTIME_FLAG,
        extra_compile_args= ["-std=c++11"] + RPATH_FLAG + CPPRUNTIMELIB_FLAG,
        runtime_library_dirs=[] if sys.platform == "win32" else ['lib'],

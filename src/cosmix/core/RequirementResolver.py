        ...
        for req in raw:
            for qty, spec in req.items():
                ...
                if isinstance(spec, dict) and "k" in spec:
                    ...
                    if qty not in merged:
                        merged[qty] = {"k": k, "z": z, "metadata": {"type": "grid_2d"}}
                    else:
                        # take union of k and z grids across likelihoods
                        merged[qty]["k"] = np.unique(np.concatenate([merged[qty]["k"], k]))
                        merged[qty]["z"] = np.unique(np.concatenate([merged[qty]["z"], z]))


        for q in merged:
            if "k" not in merged[q]:
